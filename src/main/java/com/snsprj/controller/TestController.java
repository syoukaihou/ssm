...
import com.snsprj.services.HttpRequest;
...
public class TestController {
	...
	public String testPost(){
		...
		JSONArray vehicleArray = JSON.parseArray(vehicles.get("vehicles").toString());
		
		vehicleArray.get(0);
		vehicleArray.get(1);
		
		JSONObject loadometer = JSON.parseObject(vehicleArray.get(1).toString());
		...
		StringBuffer gpsParam = new StringBuffer("version=1&method=loadLocation");
		...
		JSONObject locationObject = JSON.parseObject(JSON.parseArray(locationObjectTemp.toString()).get(0).toString());
